...
import { QrCode, UserCircle, CheckCircle, RefreshCw, Wifi } from 'lucide-react';

// ─────────────────────────────────────────────────────────────────
// Live preview samples — purely client-side, zero API calls
// Contact: Mom | Her message: "beta kha liya?"
// ─────────────────────────────────────────────────────────────────
...
const Onboarding: React.FC = () => {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      toast.success('All set! Welcome to Presenz 🎉');
      navigate('/dashboard');
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center py-12 px-4">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Set up your AI assistant in 2 quick steps</p>
        </div>
        ...
        <div className="flex items-center mb-8">
          {[
            ...
            <React.Fragment key={step.num}>
              ...
              {idx < 1 && (
                <div className={`flex-1 h-0.5 mb-5 mx-2 transition-all ${
          ...
          <div className="card p-8">
            ...
            {sessionStatus === 'pending_qr' && (
              <div className="text-center">
                ...
                <button
                  onClick={() => { setSessionStatus('idle'); setQrCode(null); if (pollingRef.current) clearInterval(pollingRef.current); }}
              ...
              <div>
                ...
                <div className="flex items-center justify-between mt-1">
                  <p className="text-xs text-gray-400">
                    Be specific — it makes AI sound genuinely like you
                  </p>
                  <span className={`text-xs font-medium ${
                    charCount < 20 ? 'text-red-400' : 'text-gray-400'
                  }`}>
                    {charCount}/{charLimit}
                  </span>
                </div>
                  ...
                  {TONE_OPTIONS.map((opt) => (
                    ...
                    >
                      <p className={`text-sm font-semibold ${
                        aiTone === opt.value ? 'text-primary-700' : 'text-gray-800'
                      }`}>
                        {opt.label}
                      </p>
              ...
              <div className="mb-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {LENGTH_OPTIONS.map((opt) => (
                    ...
                    >
                      <p className={`text-sm font-semibold ${
                        aiLength === opt.value ? 'text-primary-700' : 'text-gray-800'
                      }`}>
                        {opt.label}
                      </p>
                      <p className="text-xs text-gray-400 mt-0.5 leading-tight">{opt.desc}</p>
